...
void nms_sorted_bboxes(const std::vector<Object_yolact>& faceobjects, std::vector<int>& picked, float nms_threshold, bool agnostic )
{
    picked.clear();
    ...
    for (int i = 0; i < n; i++)
    {
        const Object_yolact& a = faceobjects[i];

        int keep = 1;
        for (int j = 0; j < (int)picked.size(); j++)
        {
            const Object_yolact& b = faceobjects[picked[j]];
...
int detect_yolact(const ncnn::Net& yolact,cv::Mat& bgr, std::vector<Object_yolact>& objects)
{
    ...
    const int keep_top_k = 50;

    std::vector<std::vector<Object_yolact> > class_candidates;
    ...
    {
        std::vector<Object_yolact>& candidates = class_candidates[i];
...
void draw_objects(cv::Mat& image, const std::vector<Object_yolact>& objects)
{
    ...
    {
        const Object_yolact& obj = objects[i];
